...
define(["./svgutensils", "./renderutensils", "./renderskeleton", "../text/textutensils", "../text/flatten", "../text/dotmap", "./rowmemory"],
    function(svgutl, utl, skel, txt, flatten, map, rowmemory) {
    /**
     *
    ...
    function getMaxEntityHeight(pEntities, pInititalEntityHeight) {
        ...
        pEntities.forEach(function(pEntity){
            lHeight = svgutl.getBBox(renderEntity(pEntity.name, pEntity)).height;
            if (lHeight > lHWM) {
                lHWM = lHeight;
            }
        });
    ...
    function renderEntity(pId, pEntity) {
        ...
        var lRect = svgutl.createRect({width: gChart.entityWidth, height: gChart.entityHeight});
    ...
    function _renderEntity(pEntity, pEntityXPos) {
        ...
        gChart.layer.sequence.appendChild(svgutl.createUse(pEntityXPos, 0, toId(pEntity.name)));
    ...
    function renderArcRows(pArcRows, pEntities) {
        ...
        gChart.layer.lifeline.appendChild(svgutl.createUse(0, rowmemory.get(-1).y, toId("arcrow")));
        ...
        if (pArcRows) {
            pArcRows.forEach(function(pArcRow, pRowNumber){
                ...
                gChart.layer.lifeline.appendChild(svgutl.createUse(0, rowmemory.get(pRowNumber).y, toId(lArcRowId)));
