...
describe('ui/utl/store', function () {
  ...
  describe('#load and save with no localStorage available', function () {
    it('silently fail on load', function () {
      store.load(lState)
      assert.strict.equal(lLanguage, 'initial language')
      assert.strict.equal(lSource, 'initial source')
      assert.strict.equal(lAutoRender, false)
      assert.strict.equal(lMirrorEntities, false)
    })
  ...
  describe('#load and save', function () {
    ...
    it('saves the passed state', function () {
      store.save(lState)
      store.load(lState)
      assert.strict.equal(lLanguage, 'other language')
      assert.strict.equal(lSource, 'other source')
      assert.strict.equal(lAutoRender, true)
      assert.strict.equal(lMirrorEntities, true)
    })
